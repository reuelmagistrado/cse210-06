...
class SceneManager:
    ...
    def prepare_scene(self, scene, cast, script, position):
        if scene == NEW_GAME:
            self._prepare_new_game(cast, script)
        elif scene == NEXT_LEVEL:
            self._prepare_next_level(cast, script)
        elif scene == TRY_AGAIN:
            self._prepare_try_again(cast, script, position)
        elif scene == IN_PLAY:
    ...
    def _prepare_try_again(self, cast, script, position):
        self._add_ball(cast, position)
        self._add_racket(cast)
        self._add_dialog(cast, PREP_TO_LAUNCH)
    ...
    def _add_ball(self, cast, new_position=0):
    ...
    def _add_level(self, cast):
        cast.clear_actors(PLAYER1_GROUP)
        text = Text(PLAYER1_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_LEFT)
        position = Point(HUD_MARGIN, HUD_MARGIN)
        label = Label(text, position)
        cast.add_actor(PLAYER1_GROUP, label)

    def _add_lives(self, cast):
        cast.clear_actors(PLAYER2_GROUP)
        text = Text(PLAYER2_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_RIGHT)
        position = Point(SCREEN_WIDTH - HUD_MARGIN, HUD_MARGIN)
        label = Label(text, position)
        cast.add_actor(PLAYER2_GROUP, label)
    ...
    def _add_stats(self, cast):
        ...
        cast.add_actor(STATS_GROUP, stats2)
    ...
    def _add_racket(self, cast):
        ...
        position = Point(x, y)
        size = Point(RACKET_HEIGHT, RACKET_WIDTH)
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(RACKET_IMAGES, RACKET_RATE)
        racket1 = Racket(body, animation)
        cast.add_actor(RACKET_GROUP, racket1)
